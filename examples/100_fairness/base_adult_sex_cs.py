...
# ==========================
sf = "sex"
fair_metric = set_fair_params.set_fair_metric(sf, "consistency_score")
...
print(
    "Fairness constrain",
    consistency_score(X_test, predictions),
)
############################################################################
...
with open("redlightinng_cs.txt", "w") as f:
    for idx, ensemble in enumerate(pareto_front):
        predictions = ensemble.predict(X_test)
        acc = sklearn.metrics.accuracy_score(y_test, predictions)
        fairness = consistency_score(X_test, predictions)
